	...
	(a.prototype.tokenize = function (a) {
		...
		for (b = 0, c = j.length; b < c; b++) {
			if (((d = f(j[b])), this.settings.diacritics))
				for (g in h)
					h.hasOwnProperty(g) &&
						(d = d.replace(new RegExp(g, 'g'), h[g]));
		...
		(a.prototype.search = function (a, b) {
			...
			return (
				...
				'number' == typeof b.limit &&
					(d.items = d.items.slice(0, b.limit)),
				d
			);
		});
	var b = function (a, b) {
		...
		i = (function () {
			...
			for (c in h)
				if (h.hasOwnProperty(c))
					for (
						d = h[c].substring(2, h[c].length - 1),
							e += d,
							a = 0,
							b = d.length;
						a < b;
		...
		a.mixin = function (a) {
			(a.plugins = {}),
				...
				(a.prototype.loadPlugin = function (b) {
					...
					(d.requested[b] = !0),
						(d.loaded[b] = e.fn.apply(c, [
							c.plugins.settings[b] || {},
						])),
						d.names.push(b);
				}),
				(a.prototype.require = function (a) {
					...
					if (!b.plugins.loaded.hasOwnProperty(a)) {
						if (c.requested[a])
							throw new Error(
								'Plugin has circular dependency ("' + a + '")'
							);
						b.loadPlugin(a);
					}
					return c.loaded[a];
	...
	(function (a, b) {
			...
			off: function (a, b) {
				...
				return 0 === c
					...
					: ((this._events = this._events || {}),
					  void (
							a in this._events != !1 &&
							this._events[a].splice(
								this._events[a].indexOf(b),
								1
							)
					  ));
			},
		...
		var C = function (a) {
			...
			L = function (c, d) {
				...
				(h = c[0]), (h.selectize = i);
				var j =
					window.getComputedStyle && window.getComputedStyle(h, null);
				if (
					((g = j
				...
				(i.settings.mode =
					i.settings.mode ||
					(1 === i.settings.maxItems ? 'single' : 'multi')),
		...
		return (
			...
			a.extend(L.prototype, {
				setup: function () {
					...
					if (
						...
						(d = a('<input type="text" autocomplete="off" />')
							...
							.attr(
								'tabindex',
								q.is(':disabled') ? '-1' : l.tabIndex
							)),
						(h = a(m.dropdownParent || b)),
						(e = a('<div>')
							...
							.appendTo(h)),
						(g = a('<div>')
							.addClass(m.dropdownContentClass)
							.appendTo(e)),
						l.settings.copyClassesToDropdown && e.addClass(j),
						b.css({ width: q[0].style.width }),
						l.plugins.names.length &&
							((k = 'plugin-' + l.plugins.names.join(' plugin-')),
							b.addClass(k),
							e.addClass(k)),
						(null === m.maxItems || m.maxItems > 1) &&
							l.tagType === v &&
							q.attr('multiple', 'multiple'),
						l.settings.placeholder &&
							d.attr('placeholder', m.placeholder),
						!l.settings.splitOn && l.settings.delimiter)
					) {
						var u = l.settings.delimiter.replace(
							/[-\/\\^$*+?.()|[\]{}]/g,
							'\\$&'
						);
					...
					q.attr('autocorrect') &&
						d.attr('autocorrect', q.attr('autocorrect')),
						q.attr('autocapitalize') &&
							d.attr('autocapitalize', q.attr('autocapitalize')),
						(l.$wrapper = b),
						(l.$control = c),
						(l.$control_input = d),
						(l.$dropdown = e),
						(l.$dropdown_content = g),
						...
						p.on('keydown' + n, function (a) {
							(l.isCmdDown = a[f ? 'metaKey' : 'ctrlKey']),
								(l.isCtrlDown = a[f ? 'altKey' : 'ctrlKey']),
								(l.isShiftDown = a.shiftKey);
						}),
						p.on('keyup' + n, function (a) {
							a.keyCode === t && (l.isCtrlDown = !1),
								a.keyCode === r && (l.isShiftDown = !1),
								a.keyCode === s && (l.isCmdDown = !1);
						}),
						p.on('mousedown' + n, function (a) {
							if (l.isFocused) {
								if (
									a.target === l.$dropdown[0] ||
									a.target.parentNode === l.$dropdown[0]
								)
									return !1;
								l.$control.has(a.target).length ||
									a.target === l.$control[0] ||
									l.blur(a.target);
							}
						}),
						o.on(
							['scroll' + n, 'resize' + n].join(' '),
							function () {
								l.isOpen &&
									l.positionDropdown.apply(l, arguments);
							}
						),
						...
						q.attr('tabindex', -1).hide().after(l.$wrapper),
						a.isArray(m.items) &&
							(l.setValue(m.items), delete m.items),
						x &&
							q.on('invalid' + n, function (a) {
								a.preventDefault(),
									(l.isInvalid = !0),
									l.refreshState();
							}),
						...
						l.updatePlaceholder(),
						(l.isSetup = !0),
						q.is(':disabled') && l.disable(),
						l.on('change', this.onChange),
						q.data('selectize', l),
						q.addClass('selectized'),
						l.trigger('initialize'),
						m.preload === !0 && l.onSearchChange('');
				},
				setupTemplates: function () {
					var b = this,
						...
						e = {
							...
							option: function (a, b) {
								return (
									'<div class="option">' + b(a[c]) + '</div>'
								);
							},
							item: function (a, b) {
								return (
									'<div class="item">' + b(a[c]) + '</div>'
								);
							},
				...
				onPaste: function (b) {
					var c = this;
					c.isFull() || c.isInputHidden || c.isLocked
						? b.preventDefault()
						: c.settings.splitOn &&
						  setTimeout(function () {
								for (
									var b = a
											.trim(c.$control_input.val() || '')
											.split(c.settings.splitOn),
										d = 0,
										e = b.length;
									d < e;
									d++
						...
						if (
							...
							for (
								k = l.$control.children('.active:last'),
									g = Array.prototype.indexOf.apply(
										l.$control[0].childNodes,
										[k[0]]
									),
									h = Array.prototype.indexOf.apply(
										l.$control[0].childNodes,
										[b[0]]
									),
									g > h && ((j = g), (g = h), (h = j)),
				...
				search: function (b) {
					...
					if (
						g.score &&
						((e = f.settings.score.apply(this, [b])),
						'function' != typeof e)
					)
						throw new Error(
					...
					if (
						...
						for (c = d.items.length - 1; c >= 0; c--)
							f.items.indexOf(z(d.items[c].id)) !== -1 &&
								d.items.splice(c, 1);
					return d;
				},
				refreshOptions: function (b) {
					...
					var t = this,
						...
						x =
							t.$activeOption &&
							z(t.$activeOption.attr('data-value'));
					for (
						g = v.items.length,
						...
						for (
							...
							(l = m[e]),
								...
								h.hasOwnProperty(l) ||
									((h[l] = document.createDocumentFragment()),
									i.push(l)),
								h[l].appendChild(k);
					for (
					...
					if (
						...
						for (c = 0, g = v.tokens.length; c < g; c++)
				...
				addItems: function (b, c) {
					for (
						var d = a.isArray(b) ? b : [b], e = 0, f = d.length;
						e < f;
						e++
					)
							...
							i.items.indexOf(b) !== -1
								...
								: void (
										...
										('multi' === j && i.isFull()) ||
											...
											i.isSetup &&
												((f =
													i.$dropdown_content.find(
														'[data-selectable]'
													)),
												i.isPending ||
													((e = i.getOption(b)),
													(g = i
														.getAdjacentOption(e, 1)
														.attr('data-value')),
													i.refreshOptions(
														i.isFocused &&
															'single' !== j
				...
				refreshState: function () {
					var a,
						b = this;
					b.isRequired &&
						(b.items.length && (b.isInvalid = !1),
						b.$control_input.prop('required', a)),
						b.refreshClasses();
				},
				...
				updateOriginalInput: function (a) {
					...
					if (((a = a || {}), f.tagType === v)) {
						for (d = [], b = 0, c = f.items.length; b < c; b++)
							(e =
								f.options[f.items[b]][f.settings.labelField] ||
								''),
								d.push(
									'<option value="' +
										A(f.items[b]) +
					...
					f.isSetup &&
						(a.silent || f.trigger('change', f.$input.val()));
				},
				...
				deleteSelection: function (b) {
					...
					if (
						...
						l.$activeOption &&
							...
							(i = l
								.getAdjacentOption(l.$activeOption, -1)
								.attr('data-value')),
						(g = []),
						l.$activeItems.length)
					) {
						for (
							k = l.$control.children(
				...
				setCaret: function (b) {
					...
					if (
						...
						var d, e, f, g;
						for (
							f = c.$control.children(':not(input)'),
								d = 0,
								e = f.length;
							d < e;
							d++
				...
				render: function (b, c) {
					...
					return (
						...
						h.renderCache[b].hasOwnProperty(d))
							? h.renderCache[b][d]
							: ((f = a(
									h.settings.render[b].apply(this, [c, A])
							  )),
							  'option' === b || 'option_create' === b
									? f.attr('data-selectable', '')
									: 'optgroup' === b &&
									  ((e =
											c[h.settings.optgroupValueField] ||
											''),
									  f.attr('data-group', e)),
							  ('option' !== b && 'item' !== b) ||
									f.attr('data-value', d || ''),
				...
				var c = a.fn.selectize.defaults,
					...
					k = function (b, c) {
						...
						if (l)
							...
							for (
								j = m.split(d.delimiter), h = 0, i = j.length;
								h < i;
								h++
							)
								(k = {}),
				...
				(b = a.extend({ equalizeWidth: !0, equalizeHeight: !0 }, b)),
					...
					(this.onKeyDown = (function () {
						...
						return function (b) {
							var d, e, f, g;
							return !this.isOpen ||
								(b.keyCode !== j && b.keyCode !== m)
								? a.apply(this, arguments)
								: ((c.ignoreHover = !0),
								  (g =
										this.$activeOption.closest(
											'[data-group]'
										)),
								  (d = g
										.find('[data-selectable]')
